...
export async function generateUI(prompt: string, style: string, imageBase64?: string): Promise<WrappedGenerationResult> {
  // 1. Top-Level Error Trap
  try {
    console.log("[ServerAction] Starting generateUI with Groq...");

    const apiKey = process.env.GROQ_API_KEY;
    if (!apiKey) {
      console.error("[ServerAction] Error: GROQ_API_KEY is missing");
    ...
    console.log(`[ServerAction] Using model: ${modelName}`);
    ...
    try {
      ...
      console.error("[ServerAction] JSON Parse Error:", e, content);
      throw new Error("Invalid JSON generated by model");
    }
